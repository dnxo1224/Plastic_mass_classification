...
file_name = 'Product_PP_POS_Bi3_005_0.txt'
df = pd.read_csv(file_path+file_name, sep='\s+', skiprows=3, names=['Channel', 'm/z', 'Intensity'])

print("Column names:", df.columns.tolist())

peaks, properties = find_peaks(df['Intensity'], height=1500, distance=1000)

peak_masses = df['m/z'].iloc[peaks]
...
# 점선으로 그리기
plt.plot(df['m/z'], df['Intensity'], 'o', markersize=0.5, color='black', alpha=0.5, label='Raw Data (Points)')
...
plt.xlim(43, 43.25)  # 보고 싶은 질량 범위 설정
